...
def get_symbol(num_classes, num_filter, model_name, **kwargs):
    ...
    output_layer = stage(4, 8, model_name, output_layer, num_filter * 2, num_filter * 8)
    output_layer = stage(12, 36, model_name, output_layer, num_filter * 4, num_filter * 16)
    output_layer = stage(48, 3, model_name, output_layer, num_filter * 8, num_filter * 32)

    res50_eletwise_bn = mx.symbol.BatchNorm(name='res50_eletwise_bn', data=output_layer, use_global_stats=use_global_stats,
                                            fix_gamma=False, eps=0.00001)
    res50_eletwise_scale = res50_eletwise_bn
    res50_eletwise_relu = mx.symbol.Activation(name='res50_eletwise_relu', data=res50_eletwise_scale, act_type='relu')
    ########## avg_pool #########
    avg_pool = mx.symbol.Pooling(name='avg_pool', data=res50_eletwise_relu, kernel=(1, 1), pool_type='avg',
                              global_pool=True,
...
softmax.save('resnet152-v2-test-symbol.json')
