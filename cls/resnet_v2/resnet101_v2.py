...
def stage(start_num, num_block, model_name, output_layer, input, output):
    for i in xrange(num_block):
        if start_num == 1:
            if i == 0:
                res1_match_conv = mx.symbol.Convolution(name='res1_match_conv', data=output_layer, num_filter=output, pad=(0, 0),
                                                        kernel=(1, 1),
                                                        stride=(1, 1), no_bias=True)
                ...
                res1_conv2 = mx.symbol.Convolution(name='res1_conv2', data=res1_conv1_relu, num_filter=input, pad=(1, 1),
                                                   kernel=(3, 3),
                                                   stride=(1, 1), no_bias=True)
                ...
                res1_conv3 = mx.symbol.Convolution(name='res1_conv3', data=res1_conv2_relu, num_filter=output, pad=(0, 0),
                                                   kernel=(1, 1),
                                                   stride=(1, 1), no_bias=True)
...
def get_symbol(num_classes, num_filter, model_name, **kwargs):
    ...
    conv1 = mx.symbol.Convolution(name='conv1', data=data, num_filter=num_filter, pad=(3, 3), kernel=(7, 7), stride=(2, 2),
                                  no_bias=True)
    ...
    output_layer = stage(4, 4, model_name, output_layer, num_filter * 2, num_filter * 8)
    output_layer = stage(8, 23, model_name, output_layer, num_filter * 4, num_filter * 16)
    output_layer = stage(31, 3, model_name, output_layer, num_filter * 8, num_filter * 32)

    res33_eletwise_bn = mx.symbol.BatchNorm(name='res33_eletwise_bn', data=output_layer, use_global_stats=use_global_stats,
                                            fix_gamma=False, eps=0.00001)
    res33_eletwise_scale = res33_eletwise_bn
    res33_eletwise_relu = mx.symbol.Activation(name='res33_eletwise_relu', data=res33_eletwise_scale, act_type='relu')
    ########## avg_pool #########
    avg_pool = mx.symbol.Pooling(name='avg_pool', data=res33_eletwise_relu, kernel=(1, 1), pool_type='avg',
                              global_pool=True,
...
softmax.save('resnet101-v2-symbol.json')
